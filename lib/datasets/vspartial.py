...
class VsPartial(BaseDataset):
    def __init__(self,
        ...
        super(VsPartial, self).__init__(ignore_label, base_size, crop_size, downsample_rate, scale_factor, mean, std)
        ...
        self.imgs = os.listdir(os.path.join(self.rootpth, self.mode, 'jpg_partial'))
    ...
    def __getitem__(self, idx):
        name = self.imgs[idx]
        image = cv2.imread(osp.join(self.rootpth, self.mode, 'jpg_partial', name), cv2.IMREAD_COLOR)
        if self.mode != 'test':
            label = cv2.imread(osp.join(self.rootpth, self.mode, 'mask_partial', name[:-3] + 'png'), cv2.IMREAD_GRAYSCALE)
            size = label.shape
        else:
            size = image.shape
